...
// 在组件挂载后获取代码内容
onMounted(() => {
  fetchCode('download', content, loadingContent);
  fetchCode('download_safe', content2, loadingContent2);
});

// 运行按钮的行为
const showHealthReminder = () => {
  ElMessageBox.alert('在点击下载后，用Burp抓包，然后修改文件名字段，修改为../flag.txt, 下载到根目录的flag就算利用成功', '关于如何利用', {
    confirmButtonText: '确定',
    type: 'warning',
